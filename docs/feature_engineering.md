...
| exp_heavy_kaggleB  | +Heavy FE batch1 (25 feats, ADR #035) | 0.952514            | **Δ −0.00130 (큰 음수=희석)** — Heavy FE 첫 배치, 25개 일괄. fold0 importance: 신피처 24/25개 하위권(**position 동역학·mask·expanding-delta = 노이즈**, eda_05 sparsity 부패와 일치=시계열 FE 안 먹음). **유일 신호축 = 횡단면 group-relative**: `tyrelife_rank_in_race_compound`(split랭크9·gain랭크10/44, split=402/gain=14588 — split·gain 공히 신피처 중 유일 top12)·`laptime_vs_race_median`·`laptime_vs_driver_median`·`tyrelife_vs_compound_median`(이상 split랭크 13/22/24). → 횡단면으로 prune 재테스트(exp_xsec). is_consec_lap(43위)·poschange도 여기서 노이즈 재확인 |

| exp_xsec_kaggleB   | +횡단면 group-relative 11 (batch1 prune) | 0.953437            | **Δ −0.00038 (음수)** — batch1 −0.0013 → prune −0.0004로 손상↓이나 여전히 음수. **다양성 축도 실패**: 잔차상관 LGBM **0.9945**(오히려↑), 스택 add **+0.00001**(노이즈)·swap −0.00007. `tyrelife_rank`(split=402/gain=14588) 등 importance 있어도 OOF·스택 무기여 = GBDT가 raw+Driver TE+i_*로 동일정보 추출 → **횡단면 테마조차 흡수** |
| exp_combo_{lgbm,xgb,cat} | +Heavy FE 조합형 **215** (키5×수치5×집계8, 피처-only, ADR #037) | 0.9509~0.9515 (단일) | **stack-add +0.000001 (기각·park)** — 대규모분기 stacking-channel(HEAVY_FE §5) 실측. 단일모델 −0.0023~0.0029(설계대로, 판정 제외). **ridge-logistic 전 C(1.0~0.01) +0.000000~+0.000001**(정규화 메타 H2 실패). combo OOF: 5멤버 회귀 **R²=0.979**·**잔차 AUC 0.49(=랜덤·신호 없음)**·corr 0.982~0.989. logistic coef combo 0.065/−0.018/−0.072(메타 거의 안 씀·음수). = **FE 마지막 채널 종결**. best_iter lgbm/xgb 수렴·cat fold3 cap접촉(무의미). 누수검증 PASS(피처-only, 단변량 AUC max 0.7465). 동반: 노트북 생성기·모니터 유틸 |
| (orig-col S1) | +orig-col TE **7** (`te_orig_*`: Compound·Stint·Compound_Stint·TyreLife/RaceProgress/LapNumber_bucket·Driver, 원본 라벨 m-est smoothed, augment OFF) | **pending (Kaggle 5-fold A/B)** | **Phase 1 S1 디코릴레이션 채널(decisions #038)** — 원본데이터(101371행, 타깃률 0.2548) 외부 라벨 TE 를 공유 키로 대회 행에 merge. Heavy FE(대회피처 집계, 흡수됨)와 정반대=대회로 재구성 불가한 외부 신호. **누수검증 PASS**: 원본↔대회 train 행 disjoint(해시 overlap=0)·te_orig NaN/inf=0(train/test/source)·train==test 매핑 일관·합성 Driver(408,871행) 전부 prior fallback·단변량 AUC 0.506~0.766(누수 아님, 0.95 미만). **1-fold 위생 스모크**(프로드 경로): LGBM augOFF fold0=0.952535(best_iter 1200<cap 5000)·augON(source 101371행 build) fold0=0.953693 무크래시 → 증강-소스 빌드 경로 커버. ⚠️ XGB augON 소스경로는 **기존** `add_xgb_freq_features` 의 category-Driver `.map().fillna(0)` 버그로 크래시(prod augOFF 무관, 미수정 보고). 풀 학습·스택 기여 미측정 |

> 📌 학습(Heavy FE batch1, 2026-06-06): 합성 sparsity 환경에선 **시계열 expanding/cumsum/mask 피처 = 노이즈**(importance 하위), **횡단면 group-relative(rank·percentile·vs-group-median)만 importance 상위**(split·gain 양쪽 교차확인, 결론 동일 — tyrelife_rank만 신피처 중 top12). 단 횡단면도 prune 후 개별 Δ−0.0004·스택 +0.00001 = **개별·다양성 양축 모두 무기여**. **종합(prevstint·pitwin·relhist·poschange·is_consec_lap·Heavy25·xsec 7전): 현 LGBM(Driver TE+i_*)에서 FE 증분은 흡수**. Driver(split=11450·gain=365k)가 신호 지배 + GBDT가 raw로 등가 추출. (참고: gain 최상위는 Year/Stint/Driver/i_tyre_over_lapnum — 범주·상호작용 분기.) 잔여 +0.00052는 FE/분산 공간 밖일 가능성(eda_03/04 환원불가 ~3% + eda_05 합성 아티팩트).
>
> 📌 학습(대규모분기 215조합, 2026-06-07): **8전째 = exp_combo(215 피처-only 조합 × GBDT 3종)** 로 HEAVY_FE_OPINION §5 의 "단일모델 ❌이나 스택 decorrelation OOF는 가치" 가설(=FE 마지막 미검증 채널)을 직접 시험 → **기각**. stack-add +0.000001·ridge 전 C 무효·잔차 AUC 0.49(노이즈)·R² 0.979 = combo OOF 가 기존 5멤버 고상관 블록으로 붕괴(#015·#035 예측대로). **FE 3축(신호·분산·스택다양성) 전부 종결 → FE 레버 park**(ADR #037). 잔여 +0.00052 는 현 표현공간 밖(베이즈-AUC 근처) 강한 증거. + meta-overfit 경고(스택 OOF 0.95436→Private 0.95400).
