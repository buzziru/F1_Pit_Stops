...
## Tier-2 — 적용 완료 (2026-06-04) — realmlp_fe → conf/features 그룹화 (#4+#5)
- **신규** `conf/features/realmlp_fe.yaml`: `feature_builder: add_realmlp_features` + `target_encode_cols: [Driver, Race_Compound, Race_Year]`(cross를 TE **단일 소스**) + smoothing/drop_cols.
- **train_realmlp.py**: 루트 `realmlp_fe` 플래그·4 분기·`features.REALMLP_CROSS_COLS` **제거** → `build()` 헬퍼가 `cfg.features.get('feature_builder')` 로 일반 훅 적용(build_features 후 `getattr(features, builder)`). te_cols 는 yaml 의 target_encode_cols 에서만.
- **conf/config.yaml**: 루트 `realmlp_fe` 제거. **features.py**: `REALMLP_CROSS_COLS` 상수 제거.
- 활성: `model=realmlp features=realmlp_fe`(기존 `realmlp_fe=true` 대체). base/driver_te(feature_builder 없음)는 None 폴백.
