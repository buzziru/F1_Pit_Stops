...
2. **인증 = Colab Secrets**(kaggle.json 업로드 불요): 좌측 🔑에 `KAGGLE_USERNAME`·`KAGGLE_KEY` 등록(노트북 액세스 ON) →
   ```python
   from google.colab import userdata; import os, json
   os.makedirs('/root/.kaggle', exist_ok=True)
   json.dump({'username': userdata.get('KAGGLE_USERNAME'), 'key': userdata.get('KAGGLE_KEY')}, open('/root/.kaggle/kaggle.json','w'))
...
6. **run**: `cfg=OmegaConf.create({...features=base, model=..., max_folds:1})` → `run(cfg)`.
