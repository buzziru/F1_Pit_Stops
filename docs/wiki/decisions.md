...
- **이유(5멤버 OOF 잔차상관 진단, `scripts/diag_resid_corr.py`)**:
  - **유효 독립 모델 수 N_eff = 1.03** (오차상관 평균 0.967, K=5). GBDT×2 + RealMLP + CatBoost + TabICL = 아키텍처 5개인데 **오차 공간 독립성은 1.03개**. 가장 잘 떨어진 쌍(RealMLP↔CatB 오차상관 0.956)조차 N_eff 1.02.
  - **평균화 분산감소 이론 천장 = −2.6%** (ρ_resid 0.967). 스택이 best single(LGBM 0.9538) 대비 이미 +0.001(0.954357)을 먹음 = **천장 거의 소진**. 멤버를 더 넣거나 더 떼어도 0.9744 부근 한계.
  - 예측상관(0.977) ≈ 오차상관(0.967) ≈ 랭크-오차상관(0.971): **"다양해 보이지만 실수는 97% 공유"**. 떼어낸 corr 3%가 신호 아닌 노이즈 → 축①이 corr 0.993→0.975 분기에 성공하고도 스택 기여 0이었던 이유 실증.
  - **오차의 환원불가 바닥**: 오차의 79%가 비경계(평균오차 0.065)로 사실상 풀림, 전 오차가 **경계영역 20.7%(평균오차 0.433)에 집중**. 그중 **합의-오답 3.19%**(5모델 std<0.05인데 |y−p̄|>0.5) = 전 아키텍처가 만장일치로 틀리는 영역 = 라벨노이즈/전략 서프라이즈. **모델 다양성이 절대 못 건드림**.
- **회고(왜 신모델 축이 기대만큼 안 됐나)**: 타뷸러 NN(RealMLP/TabM/FTT/TabICL)은 GBDT와 **다른 함수공간이 아니라 같은 신호의 다른 근사**(설계 목표 자체가 "GBDT와 경쟁"). 모두 corr 0.969 한 밴드 수렴 = 동일 신호·다른 노이즈 시그니처. "큰 레버"라 믿은 모델 다양성 = 실측 천장 −2.6%의 **작은 레버**였음. **과거 대회에서 통한 건 블렌딩이 아니라 그 밑의 FE** — 레버는 안 변했고, 이번엔 신호 레버(FE)를 일찍 접고 분산 레버로 갈아탄 게 미스매치.
- **대안·트레이드오프**: 축①이 무용했던 건 아님 — TabICL이 LB +0.00005 낸 걸로 천장을 실측 확인. 단 N_eff 1.03 → 추가 모델·분기는 EV 음. **남은 경로 = 경계영역 3.2% 합의-오답 케이스 EDA(어떤 상황에서 전원 틀리나) → F1 도메인 피처(피트윈도 거리·갭 다이내믹스·스틴트상대 열화율)로 그 케이스 가르는 신호 탐색**. 신호가 없으면 0.95 플래토가 현 피처셋 베이즈-AUC 근처라는 뜻(목표 도달 자체가 외부데이터/킬러피처 의존).
